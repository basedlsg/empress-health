...
const PINECONE_NAMESPACE  = process.env.PINECONE_NAMESPACE || "clinical-framework";
// Pinecone-hosted embedding model used for both upsert and query. Default is
// llama-text-embed-v2 at 768-dim (matches the existing "empress" index).
const PINECONE_EMBED_MODEL = process.env.PINECONE_EMBED_MODEL || "llama-text-embed-v2";
const PINECONE_EMBED_DIM   = Number(process.env.PINECONE_EMBED_DIM || 768);
...
const SYNONYMS = {
  ...
  // Sitewide copy uses "MHT" (menopausal hormone therapy); the corpus predates
  // that rename and only contains "HRT". Bridge them both ways so a query in
  // either term reaches the hormone-therapy chunks.
...
async function init() {
  ...
  if (MODE === "pinecone") {
    ...
    _pineconeIndex = _pineconeClient.index(PINECONE_INDEX_NAME);
    console.log(`[retrieval] Pinecone mode: index="${PINECONE_INDEX_NAME}" namespace="${PINECONE_NAMESPACE}" embed="${PINECONE_EMBED_MODEL}"`);
  } else {
...
async function retrieve({ query, k = 5, filter = {} } = {}) {
  ...
  if (MODE === "pinecone") {
    // Embed the query via Pinecone's hosted Inference API. Same key, same
    // model used at upsert time, so the dim matches. No OpenAI required.
    const embedResult = await _pineconeClient.inference.embed({
    ...
    // Scope to the clinical-framework namespace so we don't see any other
    // data that lives in the shared "empress" index.
    const ns = _pineconeIndex.namespace(PINECONE_NAMESPACE);
    const result = await ns.query(queryOpts);
...
async function retrieveContext({
  ...
  for (const chunk of chunks) {
    ...
    // Surface a short evidence excerpt so the UI can show the citation text,
    // not just an opaque chunk id (the brand's visible-citation promise).
    const snippet = body.length > 240
      ? body.slice(0, 240).replace(/\s+\S*$/, "") + "…"
      : body;
    citationLines.push({ id: chunk.id, score: chunk.score, snippet });
  }
