 ...
 *  `/api/checkout` is a stub that only records paid/free entitlement on the
 *  caller's own session and returns a redirect — no payment, no cross-user
 *  data — so it is safe to exempt. The static pricing page is served without a
 *  CSRF cookie, so the checkout button has no token to send. */
// Exact route paths only — matched with Set.has(req.path), never by prefix.
// Prefix matching previously meant any future route sharing a prefix (e.g.
// "/api/loginhistory") would silently lose CSRF protection. Every entry below is
// a full route path; add new exemptions explicitly.
const EXEMPT_PATHS = new Set([
  '/api/signup',
  '/api/login',
  '/qa',
  '/api/affirmations/unsubscribe',
  '/api/checkout/start',
...
function verifyCsrfMiddleware(req, res, next) {
  // Safe methods and exempt routes pass through immediately. Exact-path match
  // only — req.path excludes the query string, so query params don't matter.
