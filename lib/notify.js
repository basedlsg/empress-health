...
async function notify(type, payload) {
  ...
  const filename =
    type === "referral"
      ...
      ? "gift-claims.jsonl"
      : type === "lead"
      ? "free-score-leads.jsonl"
      : "assessment-feedback.jsonl";
  // Await the write so a failure (e.g. read-only serverless FS) is observed and
  // handled here rather than surfacing as an unhandled promise rejection. The
  // write is best-effort: a failure must not block the console log / email hook.
  let filepath = null;
  try {
    filepath = await appendJsonl(filename, record);
  } catch (err) {
    console.warn(`[notify:${type}] persist to JSONL failed (non-fatal):`, err && err.message);
  }
  ...
  if (type === "feedback") {
    ...
    console.log(
      `[notify:lead] track=${record.track || record.source || "menopause"} firstName="${record.firstName || ""}" email=${record.email || ""} stage=${record.stage || "?"} score=${record.his ?? "?"}`
    );
  } else {
