...
// Site palette — matches the assessment SPA after the F8 theme alignment.
const PLUM = "3f1449";
const PLUM_LIGHT = "4b2577";
const GOLD = "D8A738";
const MUTED = "6e6a7a";
...
function p(text, opts = {}) {
  const {
    ...
    spaceBefore = 0,
    spaceAfter = 120,
    font = 'Avenir', 'Avenir Next', 'Nunito Sans',
  } = opts;
...
function h1(text) {
  return new Paragraph({
    ...
    children: [
      new TextRun({ text, bold: true, color: PLUM, size: 40, font: 'The Seasons', 'Playfair Display' }),
    ],
  });
}
...
function h2(text) {
  return new Paragraph({
    ...
    children: [
      new TextRun({ text, bold: true, color: PLUM, size: 28, font: 'The Seasons', 'Playfair Display' }),
    ],
  });
}
...
function bullet(text, ref = "bullets") {
  return new Paragraph({
    ...
    children: [new TextRun({ text, font: 'Avenir', 'Avenir Next', 'Nunito Sans', size: 22 })],
  });
}
...
function pageOne(data) {
  ...
  return [
    ...
    p(
      ...
      { italic: true, color: MUTED, size: 26, spaceAfter: 320, font: 'The Seasons', 'Playfair Display' },
    ),
    p(
    ...
    new Paragraph({
      ...
      children: [
        new TextRun({
          ...
          color: PLUM,
          bold: true,
          font: 'The Seasons', 'Playfair Display',
          size: 96,
        }),
        new TextRun({
          text: "  /100",
    ...
    p(
      ...
      { color: MUTED, size: 20, italic: true, spaceAfter: 240 },
    ),
...
function pageTwo(data) {
  ...
  items.forEach((cs, i) => {
    ...
    if (lever) {
      blocks.push(p("One small lever:", { bold: true, color: PLUM_LIGHT, size: 22, spaceAfter: 60 }));
...
function pageThree(data) {
  ...
  if (items.length === 0) {
    blocks.push(p("Your scores were fairly even across categories — there isn't a clear standout strength yet, and that's okay. As you address the priorities on Page 02, expect strengths to emerge over the next few months.", {
      color: MUTED,
      italic: true,
    }));
    return blocks;
...
function pageFour(data) {
  const { user = {}, affirmations = [], recommendations = [] } = data;
  const firstName = (user.firstName && String(user.firstName).trim()) || "you";

  const aff = (affirmations[0] && String(affirmations[0]).trim()) || null;
  const rec = recommendations[0] || null;

  const blocks = [
    pageBreak(),
    eyebrow("PAGE 04"),
    h1("Something to carry with you. Someone who could help."),
  ];

  // Affirmation block
  blocks.push(h2("Your affirmation"));
  if (aff) {
    ...
    blocks.push(p("Your personalised affirmation is being prepared and will arrive in your inbox.", {
  ...
  if (rec) {
    ...
    if (rec.reason) {
      blocks.push(p(rec.reason, { size: 22, spaceAfter: 160 }));
    }
    blocks.push(p(
...
function pageFive(data) {
  ...
  return [
    ...
    h2("02 · Pick one lever from Page 02 and run it for 14 days."),
    p(
    ...
    p(
      "This was the 30-question preview. The full 120-question assessment maps every body system in clinical detail and produces a 20-page report you can hand to a clinician. When you're ready: empresshealth.ai/membershipoptions",
      { size: 22, spaceAfter: 240 },
    ),
    ...
    p(
      ...
      { color: PLUM, italic: true, size: 22, align: AlignmentType.CENTER, spaceBefore: 240 },
    ),
  ];
}
...
function pageSix(data) {
  ...
  evidence_refs.forEach((ref) => {
    ...
    blocks.push(p(ref.description || "", { size: 22, spaceAfter: 160 }));
  });

  blocks.push(
    p(
      "Empress Health 120-Symptom Biomarker Framework v1.0 · March 2026",
      { color: MUTED, italic: true, size: 18, align: AlignmentType.CENTER, spaceBefore: 160, spaceAfter: 0 },
    ),
  );
...
async function generateFreeReport(data) {
  ...
  const doc = new Document({
    ...
    description: "Personalised free-tier 5-page report from the 30-question assessment.",
    styles: {
      default: { document: { run: { font: 'Avenir', 'Avenir Next', 'Nunito Sans', size: 22 } } },
      paragraphStyles: [
        {
          id: "Heading1",
          name: "Heading 1",
          ...
          next: "Normal",
          quickFormat: true,
          run: { size: 40, bold: true, font: 'The Seasons', 'Playfair Display', color: PLUM },
        ...
        },
        {
          ...
          next: "Normal",
          quickFormat: true,
          run: { size: 28, bold: true, font: 'The Seasons', 'Playfair Display', color: PLUM },
